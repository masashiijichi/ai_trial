...
# Gemini 3.1 Pro - WITH web search (Google Search tool)
...
# 3. API function -----------------------------------------------
get_gemini_response <- function(prompt, model = "gemini-3.1-pro") {
  ...
  for (attempt in 1:3) {
    res <- tryCatch({
      POST(url, content_type_json(), body = toJSON(body, auto_unbox = TRUE), timeout(180))
...
safe_call <- possibly(get_gemini_response, otherwise = NA_character_)
...
cat("\nSample output:\n\n")
first_valid <- results$gemini_response[which(!is.na(results$gemini_response))[1]]
